...
class TestComputeLuminance:
    ...
    def test_green_luminance(self):
        ...
        result = compute_luminance([0.0, 1.0, 0.0])
    ...
    def test_blue_luminance(self):
        ...
        result = compute_luminance([0.0, 0.0, 1.0])
    ...
    def test_mid_gray_luminance(self):
        ...
        result = compute_luminance([128/255, 128/255, 128/255])
        assert 0.21 < result < 0.22
    
    def test_linearize_low_values(self):
        """Test linearization of low RGB values (<= 0.03928)."""
        # Test the linearize function indirectly through compute_luminance
        low_value = 0.03
        result = compute_luminance([low_value, 0.0, 0.0])
    ...
    def test_linearize_high_values(self):
        ...
        high_value = 0.5
        result = compute_luminance([high_value, 0.0, 0.0])
...
class TestContrastRatio:
    ...
    def test_order_independence(self):
        ...
        l1, l2 = 0.2, 0.8
        result1 = contrast_ratio(l1, l2)
        result2 = contrast_ratio(l2, l1)
    ...
    def test_wcag_aa_threshold(self):
        ...
        result = contrast_ratio(l_light, l_dark)
...
class TestGenerateReadableColors:
    ...
    def test_white_background(self):
        ...
        bg_luminance = compute_luminance(background)
        for color in colors:
    ...
    def test_contrast_requirement(self):
        ...
        for background in backgrounds:
            ...
            for color in colors:
                color_luminance = compute_luminance(color)
                ratio = contrast_ratio(bg_luminance, color_luminance)
                assert ratio >= 4.5, f"Color {color} has insufficient contrast {ratio:.2f}"
...
def test_color_generation_expectations(background, expected_min_colors):
    """Test that certain backgrounds generate expected minimum number of colors."""
    colors = generate_readable_colors(background)
    assert len(colors) >= expected_min_colors, \
        f"Expected at least {expected_min_colors} colors for {background}, got {len(colors)}"


def test_deterministic_systematic_generation():
