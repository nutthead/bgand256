...
This module provides WCAG-compliant color generation capabilities for finding foreground
colors that maintain sufficient contrast against specified background colors. It implements
the Web Content Accessibility Guidelines (WCAG) 2.0 standards for color contrast ratios.

The module focuses on generating up to 256 colors that meet WCAG AA accessibility standards
(minimum 4.5:1 contrast ratio) through systematic HSL color space exploration and fallback
...
def compute_luminance(rgb):
    ...
    which accounts for human visual perception and gamma correction in the sRGB color space.
    This value is essential for determining color contrast ratios for accessibility compliance.
    ...
    Notes:
        ...
        - Gamma: Assumes sRGB gamma of approximately 2.2 (exact: 2.4 with linear segment)
    ...
    See Also:
        contrast_ratio: Uses luminance values to compute WCAG contrast ratios
...
def contrast_ratio(l1, l2):
    ...
    See Also:
        compute_luminance: Generates luminance values for this function
...
def generate_readable_colors(background_rgb):
    """Generate up to 256 foreground colors with WCAG AA compliant contrast against a background.
    
    This function systematically generates colors in HSL color space that meet the WCAG AA
    contrast ratio requirement of 4.5:1 against the provided background color. It uses a
    two-phase approach: systematic grid exploration followed by random sampling fallback.
    ...
    Returns:
        list[tuple[float, float, float]]: List of RGB color tuples in normalized [0,1] range.
    ...
    Notes:
        ...
        - Colors are returned in RGB format normalized to [0,1] range regardless of input format
        - Function is deterministic for systematic phase, non-deterministic for random fallback
